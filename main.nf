...
include { ASSEMBLY_UNICYCLER; ASSEMBLY_SHOVILL; ASSEMBLY_ASSESS; ASSEMBLY_QC;
          ASSEMBLY_QC_FALLBACK }                                                      from "$projectDir/modules/assembly"
include { GET_REF_GENOME_BWA_DB; MAPPING; SAM_TO_SORTED_BAM; SNP_CALL; HET_SNP_COUNT;
          MAPPING_QC; MAPPING_QC_FALLBACK }                                           from "$projectDir/modules/mapping"
...
include { srst2_for_res_typing; split_target_RES_seq_from_sam_file;
          split_target_RES_sequences; freebayes }                                     from './modules/res_alignments.nf'
...
include { finalise_sero_res_results; finalise_surface_typer_results;
          finalise_pbp_existing_allele_results; combine_results }                     from './modules/combine.nf'
...
workflow {
  ...
  // ── READ DISCOVERY (dedupe: prefer gz pairs) ────────────────────────────────
  ...
  // ── Basic validation → PREPROCESS → READ_QC ─────────────────────────────────
  FILE_VALIDATION(RAW_READS_ONE_ch)

  VALID_READS_ch = FILE_VALIDATION.out.result      // (id, status)
  ...
  READ_QC_PASSED_READS_ch = READ_QC.out.result
      ...
      // row = (id_qc, status, id_pre, r1, r2, unpaired)
      .filter { row -> row[1] == 'PASS' }
      .map { row ->
        def id_qc    = row[0] as String
        def r1       = row[3]
        def r2       = row[4]
        def unpaired = row[5]
        tuple(id_qc, r1, r2, unpaired)               // (id, r1, r2, unpaired)
      }

  // ── STRICT paired reads for typer modules ───────────────────────────────────
  OVERALL_QC_PASSED_PAIRED_READS_ch = READ_QC_PASSED_READS_ch
      .filter { id, r1, r2, unpaired -> r1 && r2 }
      .map    { id, r1, r2, unpaired -> tuple(id, r1, r2) }  // (id, R1, R2)
  ...
  ASSEMBLY_QC(
    ASSEMBLY_ASSESS.out.report.join(READ_QC.out.bases, failOnDuplicate: true),
    params.contigs, params.length_low, params.length_high, params.depth
  )
  ...
  MAPPING_QC(
    SAM_TO_SORTED_BAM.out.ref_coverage.join(HET_SNP_COUNT.out.result, failOnDuplicate: true),
    params.ref_coverage, params.het_snp_site
  )
  ...
  BRACKEN(GET_KRAKEN2_DB.out.path, TAXONOMY.out.report, params.read_len, params.classification_level, params.threshold)
  ...
  // ── OVERALL_QC (seed must be unique IDs) ───────────────────────────────────
  OVERALL_QC(
  ...
  GENERATE_SAMPLE_REPORT(
    RAW_READS_ONE_ch.map{ it[0] }
      ...
      .map { row ->
        def sample_id   = row[0]
        def reportPaths = row[1..-1].findAll { it != null && it != "NA" }
        [sample_id, reportPaths]
      }
  )
  ...
  if (params.gbs_res_min_coverage < 0 | params.gbs_res_min_coverage > 100){ println("--gbs_res_min_coverage out of range [0..100]"); System.exit(1) }
  if (params.gbs_res_max_divergence < 0 | params.gbs_res_max_divergence > 100){ println("--gbs_res_max_divergence out of range [0..100]"); System.exit(1) }
  other_res_min_coverage_list = params.other_res_min_coverage.toString().tokenize(' ')
  for (x in other_res_min_coverage_list){ if (x.toDouble() < 0 | x.toDouble() > 100){ println("--other_res_min_coverage value(s) out of range [0..100]"); System.exit(1) } }
  other_res_max_divergence_list = params.other_res_max_divergence.toString().tokenize(' ')
  for (x in other_res_max_divergence_list){ if (x.toDouble() < 0 | x.toDouble() > 100){ println("--other_res_max_divergence value(s) out of range [0..100]"); System.exit(1) } }
  if (params.restyper_min_read_depth < 0){ println("--restyper_min_read_depth must be >= 0"); System.exit(1) }
  if (params.serotyper_min_read_depth < 0){ println("--serotyper_min_read_depth must be >= 0"); System.exit(1) }
  if (params.mlst_min_coverage < 0 | params.mlst_min_coverage > 100){ println("--mlst_min_coverage out of range [0..100]"); System.exit(1) }
  if (params.mlst_min_read_depth < 0){ println("--mlst_min_read_depth must be >= 0"); System.exit(1) }
  if (params.surfacetyper_min_coverage < 0 | params.surfacetyper_min_coverage > 100){ println("--surfacetyper_min_coverage out of range [0..100]"); System.exit(1) }
  if (params.surfacetyper_max_divergence < 0 | params.surfacetyper_max_divergence > 100){ println("--surfacetyper_max_divergence out of range [0..100]"); System.exit(1) }
  if (params.surfacetyper_min_read_depth < 0){ println("--surfacetyper_min_read_depth must be >= 0"); System.exit(1) }

  // ── Workflows (typer modules consume STRICT paired channel) ─────────────────
  workflow GBS_RES {
    take: reads
    main:
  ...
  if (params.run_sero_res){
    ...
    GBS_RES.out.fullgenes.join(GBS_RES.out.consensus).join(OTHER_RES.out.fullgenes).set { res_files_ch }
  ...
  if (params.run_surfacetyper){
    surface_typer(
      OVERALL_QC_PASSED_PAIRED_READS_ch,
  ...
  // Ensure overall report waits on sample reports
