# Extract taxonomy QC information and determine QC result based on $KRAKEN2_REPORTa
# change the params/var to Streptococcus agalactiae/qc_sagalactiae_percentage etc
...
if [[ "$(echo "$PERCENTAGE >= $QC_AGALACTIAE_PERCENTAGE" | bc -l)" == 1 ]] && [[ "$(echo "$TOP_NON_STREP_GENUS_PERCENTAGE <= $QC_TOP_NON_STREP_GENUS_PERCENTAGE" | bc -l)" == 1 ]]; then
    TAXONOMY_QC="PASS"
else
    TAXONOMY_QC="FAIL"
fi
